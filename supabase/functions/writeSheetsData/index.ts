...
async function getGoogleToken() {
    ...
    const assertion = await new SignJWT({
        scope: SCOPES,
    } as any) // Usamos 'any' ya que Deno/JOSE tienen tipado estricto
...
serve(async (req) => {
    try {
        ...
        // 2. LÓGICA DE ESCRITURA REAL A GOOGLE SHEETS
        
        // Determinar la hoja de destino
        const sheetName = data_type === 'asistencias' ? 'Asistencia' : 'Calificaciones';
        // Usamos :append para añadir una nueva fila al final de la hoja.
        const range = `${sheetName}!A1:Z`; 
        
        // 3. ESTRUCTURAR LOS DATOS (Ejemplo conceptual para Asistencia o Notas)
        // En un entorno de producción, la estructura debe coincidir exactamente con el formato de tu hoja.
        const valuesToWrite = [
            [new Date().toISOString().split('T')[0], unit, JSON.stringify(sheet_data)] // Ejemplo de fila de datos
        ];
        ...
        return new Response(
            JSON.stringify({ data: { success: true, message: `Data written to Sheet ID: ${SHEET_ID}. Type: ${data_type}` } }),
